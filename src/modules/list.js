            ...
            if (v.length > 0) {
              ...
              if (page > -1 && arrayChunks.length > page) {
                interaction.reply({ embeds: [
                  ...
                  .setDescription(`${arrayChunks[page].map((warning, index) => `**${(page * 5) + (index + 1)}) ${warning.reason}**\n└  ‎Points: ${warning.points}‎ | By: <@${warning.issuer}> | Time: ${(warning.unixTime && warning.unixTime !== 0) ? moment.unix(warning.unixTime).utc().tz('Australia/Brisbane').format('MMMM Do YYYY, h:mm a') : 'Unknown'}`).join('\n\n')}`)
